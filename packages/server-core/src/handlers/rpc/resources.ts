...
import { getCredentialManager, SOURCE_CREDENTIAL_TYPES } from '@poly-agents/shared/credentials'
...
} from '@poly-agents/shared/resources'
...
export function registerResourcesHandlers(server: RpcServer, deps: HandlerDeps): void {
  ...
  server.handle(
    ...
    async (_ctx, workspaceId: string, options: ExportResourcesOptions) => {
      ...
      const { exportResources } = await import('@poly-agents/shared/resources')
  ...
  server.handle(
    ...
    async (_ctx, workspaceId: string, bundle: ResourceBundle, mode: ResourceImportMode) => {
      ...
      const { importResources } = await import('@poly-agents/shared/resources')
