...
import type { AuthRequest } from '@poly-agents/session-tools-core';
...
export interface SessionScopedToolCallbacks {
  ...
  getSessionInfoFn?: (sessionId?: string) => import('@poly-agents/session-tools-core').SessionInfo | null;
  /** List sessions in the workspace with pagination. */
  listSessionsFn?: (options?: import('@poly-agents/session-tools-core').ListSessionsOptions) => import('@poly-agents/session-tools-core').ListSessionsResult;
  /** Resolve label display names to IDs. */
  resolveLabelsFn?: (labels: string[]) => import('@poly-agents/session-tools-core').ResolvedLabelsResult;
  /** Resolve a status display name to its ID. */
  resolveStatusFn?: (status: string) => import('@poly-agents/session-tools-core').ResolvedStatusResult;
