...
const originalCliFlag = process.env.CRAFT_FEATURE_POLY_AGENTS_CLI
    ...
    JSON.stringify(
      {
        version: '2026-03-07',
        allowedBashPatterns: [
          { pattern: '^poly-agent\\s+label\\s+list\\b', comment: 'poly-agent label read-only operations' },
...
afterEach(() => {
  ...
  if (originalCliFlag === undefined) delete process.env.CRAFT_FEATURE_POLY_AGENTS_CLI
  else process.env.CRAFT_FEATURE_POLY_AGENTS_CLI = originalCliFlag
})

describe('permissions config poly-agents-cli feature flag', () => {
  it('skips compiling poly-agent bash allowlist patterns when feature is disabled', () => {
  ...
  it('compiles poly-agent bash allowlist patterns when feature is enabled', () => {
    const tempConfigDir = mkdtempSync(join(tmpdir(), 'craft-permissions-'))
    try {
      process.env.CRAFT_CONFIG_DIR = tempConfigDir
      process.env.CRAFT_FEATURE_POLY_AGENTS_CLI = '1'
      writeDefaultPermissions(tempConfigDir)
      ...
      const sources = merged.readOnlyBashPatterns.map(p => p.source)
      expect(sources).toContain('^poly-agent\\s+label\\s+list\\b')
      expect(sources).toContain('^rg\\b')
    } finally {
      rmSync(tempConfigDir, { recursive: true, force: true })
    }
