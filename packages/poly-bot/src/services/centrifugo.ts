...
import { getWebSocketConstructorForProxy } from '../proxiedWebSocket';
...
// SX GET /orders is easy to trip rate/WAF limits when every order_book channel re-seeds at once.
const ORDER_BOOK_SEED_CONCURRENCY = 4;
let orderSeedActive = 0;
const orderSeedWait: Array<() => void> = [];

function enqueueOrderSeed(job: () => Promise<void>): Promise<void> {
  return new Promise((resolve, reject) => {
    const start = () => {
      ...
      job()
        ...
        .finally(() => {
          orderSeedActive -= 1;
          const next = orderSeedWait.shift();
          if (next) next();
        });
    };
    if (orderSeedActive < ORDER_BOOK_SEED_CONCURRENCY) void start();
    else orderSeedWait.push(start);
  });
}

let sxOrderBook403Logged = false;
let sxBestOdds403Logged = false;
...
async function seedBestOdds(): Promise<void> {
  ...
  for (let i = 0; i < hashes.length; i += SEED_BATCH_SIZE) {
    ...
    const res = await platformFetch(url, { headers: { 'x-api-key': config.SX_BET_API_KEY } });
    if (!res.ok) {
      if (res.status === 403) {
        if (!sxBestOdds403Logged) {
          sxBestOdds403Logged = true;
          const bodyText = await res.text().catch(() => '');
          log.error(
            {
              status: 403,
              bodyPreview: bodyText.slice(0, 600),
              hint: 'SX GET /orders/odds/best 403 — check SX_BET_API_KEY, proxy, and account access. Further 403s in this batch at debug.',
            },
            'best_odds REST seed failed',
          );
        } else {
          log.debug({ status: 403 }, 'best_odds REST seed failed');
        }
      } else {
  ...
  sxBestOdds403Logged = false;
  log.info({ count: hashes.length }, 'seeded markets');
}

async function seedMarkets(): Promise<void> {
  const res = await platformFetch(`${config.SX_BET_API_URL}/markets/active`, {
...
// REST seed for a single market's order book (queued — avoids stampedes on subscribe).
async function seedMarketBook(marketHash: string): Promise<void> {
  return enqueueOrderSeed(() => seedMarketBookOnce(marketHash));
}

async function seedMarketBookOnce(marketHash: string): Promise<void> {
  const url = `${config.SX_BET_API_URL}/orders?marketHashes=${marketHash}&baseToken=${USDC_BASE_TOKEN}`;
  const res = await platformFetch(url, { headers: { 'x-api-key': config.SX_BET_API_KEY } });
  if (!res.ok) {
    if (res.status === 403) {
      if (!sxOrderBook403Logged) {
        sxOrderBook403Logged = true;
        const bodyText = await res.text().catch(() => '');
        log.error(
          {
            marketHash,
            status: 403,
            bodyPreview: bodyText.slice(0, 600),
            hint: 'SX GET /orders 403 — invalid API key, geo/IP block, proxy altering traffic, or rate/WAF. Further 403s at debug.',
          },
          'order_book REST seed failed',
        );
      } else {
        log.debug({ marketHash, status: 403 }, 'order_book REST seed failed');
      }
    } else {
...
export function startCentrifugoService(): void {
  if (!config.SX_BET_API_KEY?.trim()) {
    log.warn('SX_BET_API_KEY missing — skipping Centrifugo (SX realtime disabled)');
    return;
  }
  ...
  const client = new Centrifuge(config.SX_BET_WS_URL, {
    websocket: getWebSocketConstructorForProxy() as unknown as typeof globalThis.WebSocket,
    getToken: async () => {
      const res = await platformFetch(`${config.SX_BET_API_URL}/user/realtime-token/api-key`, {
        headers: { 'x-api-key': config.SX_BET_API_KEY },
      });
      if (res.status === 401) {
  ...
  client.on('connected', () => {
    log.info({ viaProxy: Boolean(config.HTTP_PLATFORM_PROXY_URL) }, 'centrifugo connected');
  });
