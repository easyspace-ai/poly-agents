...
const polyElectron = process.env.POLY_ELECTRON === '1';
...
/** Relaxed env when embedded in Craft Electron (DATABASE_URL set by main; SX optional until user configures). */
const electronEnvSchema = z.object({
  ...
  SX_BET_API_KEY: z.preprocess((val) => {
...
const parsed = polyElectron
  ? electronEnvSchema.safeParse(process.env)
...
if (!parsed.success) {
  ...
  console.error(
    polyElectron
      ? '[config] POLY_ELECTRON: ensure DATABASE_URL is set before loading poly-bot.'
      : '[config] Edit defaults in packages/poly-bot/src/embeddedEnv.ts (or set process.env for tests/CI).',
  );
  process.exit(1);
}
