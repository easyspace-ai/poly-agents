...
import { Tooltip, TooltipTrigger, TooltipContent, DocumentFormattedMarkdownOverlay } from "@poly-agents/ui"
import {
  DropdownMenu,
  DropdownMenuTrigger,
  DropdownMenuSub,
...
import { buildLabelTree, getDescendantIds, getLabelDisplayName, flattenLabels, extractLabelId, findLabelById, sortLabelsForDisplay } from "@poly-agents/shared/labels"
import type { LabelConfig, LabelTreeNode } from "@poly-agents/shared/labels"
import { resolveEntityColor } from "@poly-agents/shared/colors"
...
function AppShellContent({
  ...
  const unifiedSidebarItems = React.useMemo((): SidebarItem[] => {
    return [
      { id: 'nav:poly:markets', type: 'nav', action: () => navigate(routes.view.poly('markets')) },
      { id: 'nav:poly:risk', type: 'nav', action: () => navigate(routes.view.poly('risk')) },
      { id: 'nav:poly:history', type: 'nav', action: () => navigate(routes.view.poly('history')) },
      { id: 'nav:poly:accounts', type: 'nav', action: () => navigate(routes.view.poly('accounts')) },
      { id: 'nav:settings', type: 'nav', action: () => handleSettingsClick('poly') },
    ]
  }, [handleSettingsClick])
  ...
  const listTitle = React.useMemo(() => {
    if (isPolyNavigation(navState)) {
      switch (navState.tab) {
        case 'markets': return t('spma.listTitle.markets')
        case 'risk': return t('spma.listTitle.risk')
        case 'history': return t('spma.listTitle.history')
        case 'accounts': return t('spma.listTitle.accounts')
        case 'setup': return t('spma.listTitle.setup')
        default: return t('spma.listTitle.markets')
      }
    }
          ...
          sidebarSlot={
            ...
            <div className="flex h-full flex-col select-none">
              ...
              <div className="flex-1 flex flex-col min-h-0">
                {/* Legacy: New Session + Craft sidebar (sessions, labels, sources, …). Disabled for SPMA — see docs/poly-integration/10-ui-renderer.md */}
                {false && (
                ...
                {/* SPMA primary nav (matches legacy dashboard rail) */}
                ...
                <LeftSidebar
                  ...
                  links={[
                    {
                      id: "nav:poly:markets",
                      title: t("spma.nav.markets"),
                      icon: LineChart,
                      variant: (isPolyNavigation(navState) && navState.tab === "markets") ? "default" : "ghost",
                      onClick: () => navigate(routes.view.poly("markets")),
                    },
                    {
                      id: "nav:poly:risk",
                      title: t("spma.nav.risk"),
                      icon: Shield,
                      variant: (isPolyNavigation(navState) && navState.tab === "risk") ? "default" : "ghost",
                      onClick: () => navigate(routes.view.poly("risk")),
                    },
                    {
                      id: "nav:poly:history",
                      title: t("spma.nav.history"),
                      icon: HistoryIcon,
                      variant: (isPolyNavigation(navState) && navState.tab === "history") ? "default" : "ghost",
                      onClick: () => navigate(routes.view.poly("history")),
                    },
                    {
                      id: "nav:poly:accounts",
                      title: t("spma.nav.accounts"),
                      icon: Wallet,
                      variant: (isPolyNavigation(navState) && navState.tab === "accounts") ? "default" : "ghost",
                      onClick: () => navigate(routes.view.poly("accounts")),
                    },
                    {
                      id: "nav:settings",
                      title: t("spma.nav.settings"),
                      icon: Settings,
                      variant: isSettingsNavigation(navState) ? "default" : "ghost",
                      onClick: () => handleSettingsClick("poly"),
                    },
                  ]}
                />
