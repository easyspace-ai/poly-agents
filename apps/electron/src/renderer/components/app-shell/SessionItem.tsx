...
import { Spinner } from "@poly-agents/ui"
...
import { extractLabelId } from "@poly-agents/shared/labels"

const PLATFORM_PILL: Record<'telegram' | 'whatsapp' | 'lark', { label: string; colorClass: string }> = {
  telegram: {
    label: 'Telegram',
  ...
  lark: {
    label: 'Lark',
    colorClass: 'bg-violet-500/10 text-violet-600 dark:bg-violet-400/15 dark:text-violet-300',
  },
}
        ...
        hasMessagingBinding ? (
          <div className="flex items-center gap-1">
            {sessionBindings.map((binding) => {
              const pill = PLATFORM_PILL[binding.platform as keyof typeof PLATFORM_PILL]
              if (!pill) return null
              return (
                <EntityListBadge
                  key={binding.id}
