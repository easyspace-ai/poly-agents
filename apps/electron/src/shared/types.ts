...
/** SPMA / Polymarket dashboard tabs (left rail + navigator) */
export const POLY_TABS = ['markets', 'risk', 'history', 'accounts', 'setup'] as const
export type PolyTab = (typeof POLY_TABS)[number]

export function isValidPolyTab(value: string): value is PolyTab {
  return (POLY_TABS as readonly string[]).includes(value)
}

/**
 * SPMA (poly) navigation — replaces legacy Craft sidebar for this product build.
 */
...
export const isPolyNavigation = (
  state: NavigationState
): state is PolyNavigationState => state.navigator === 'poly'
